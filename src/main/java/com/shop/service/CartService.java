...
public class CartService {
	...
	public Long addCart(CartItemDto cartItemDto, String email) {
		...
		CartItem savedCartItem = cartItemRepository.findByCartIdAndItemId(cart.getId(), item.getId()); // 현재 상품이 장바구니에
																										// 이미 들어가 있는지
																										// 조회한다.
		...
		if (savedCartItem != null) {
			...
			CartItem cartItem = CartItem.createCartItem(cart, item, cartItemDto.getCount()); // 장바구니 엔티티, 상품 엔티티, 장바구니에
																								// 담을 수량을 이용하여 CarItem
																								// 엔티티를 생성한다.
